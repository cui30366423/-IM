...
- (void)loginSuccess:(NSString *)userName password:(NSString *)password token:(NSString *)token userId:(NSString *)userId
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UINavigationController *rootNavi = [storyboard instantiateInitialViewController];
...
//                [self loginRongCloud:userName token:response[@"result"][@"token"] password:password];
//            } failure:^(NSError *err) {
//                dispatch_async(dispatch_get_main_queue(), ^{
//                [hud hide:YES];
//                _errorMsgLb.text=@"Token无效";
...
//    if ([self validateUserName:userName userPwd:password]) {
        ...
        hud.labelText = @"登录中...";
        [hud show:YES];
        [DCHttpTool loginWithAccount:userName password:password
            success:^(id response) {
               if ([response[responseCode] intValue] == 0) {
                   
                   [self loginRongCloud:userName token:response[@"imtoken"] password:password];
  
               }else{
                   //关闭HUD
                   [hud hide:YES];
                   int _errCode = [response[responseCode] intValue];
