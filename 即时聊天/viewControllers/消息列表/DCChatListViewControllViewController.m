//
//  DCChatListViewControllViewController.m
//  即时聊天
//
...
#import "DCChatListViewControllViewController.h"

@interface DCChatListViewControllViewController ()
{
...
@implementation DCChatListViewControllViewController
...
- (void)viewDidLoad {
    ...
    //设置tableView样式
    self.conversationListTableView.separatorColor = [UIColor colorWithHexString:AppMainColor alpha:1.0f];
    ...
    self.conversationListTableView.contentInset = UIEdgeInsetsMake(0, 0, TabBarHeight, 0);
...
- (void)setNavigationItemTitleView {
    ...
    DLog(@"%lD",(long)[[RCIMClient sharedRCIMClient]getConnectionStatus]);
    ...
    {
        [super setNavigationItemTitleView];
    }
...
- (void) pushChat:(id)sender
{
     ...
     DCConversationViewController * vc = [[DCConversationViewController alloc]init];
...
- (void) pushAddFriend:(id) sender
{
    ...
    DCSearchFriendTableViewController * vc = [storyBoard instantiateViewControllerWithIdentifier:@"SearchFriend"];
...
-(void)onSelectedTableRow:(RCConversationModelType)conversationModelType conversationModel:(RCConversationModel *)model atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_isClick) {
        ...
        if (conversationModelType == RC_CONVERSATION_MODEL_TYPE_NORMAL) {
            ...
            vc.unReadMessage=model.unreadMessageCount;
            vc.enableNewComingMessageIcon=YES;
            [self.navigationController pushViewController:vc animated:YES];
